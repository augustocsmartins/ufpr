...
def cleanhtml(html):
    ...
    rules = [
        ...
        {r'<a\s+href="([^"]+)"[^>]*>.*</a>': ''},      # Show links instead of texts.
        {r'[ \t]*<[^<]*?/?>': ''},                     # Remove remaining tags.
        {r'[-_()\"#/@:<>{}`+=~|.!?,\*]': ' '},         # Remove remaining tags.
        {r'\n+': ' '},                                 # Remove remaining tags.
        {r'\d+': 'contains:number'},                   # Remove remaining tags.
...
def tokenize(text: str) -> Set[str]:
    ...
    ps = PorterStemmer()
    all_words = [ps.stem(w) for w in word_tokenize(text, preserve_line=False)]
    # all_words = re.findall("[a-z0-9']+", text)  # extract the words, and

    return set(all_words)                       # remove duplicates.

assert tokenize("Data Science is science") == {"data", "scienc", "is"}
...
class NaiveBayesClassifier:
    ...
    def predict(self, text: str) -> float:
        ...
        return prob_if_spam / (prob_if_spam + prob_if_ham) if (prob_if_spam + prob_if_ham) else 0
...
assert model.tokens == {"spam", "ham", "rule", "hello"}
assert model.spam_messages == 1
assert model.ham_messages == 2
assert model.token_spam_counts == {"spam": 1, "rule": 1}
assert model.token_ham_counts == {"ham": 2, "rule": 1, "hello": 1}
...
for filename in glob.glob(path):
    ...
    try:
        ...
        subject = mail.subject + " " + cleanhtml(mail.body)
